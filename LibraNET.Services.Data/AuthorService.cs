...
namespace LibraNET.Services.Data
{
    public class AuthorService : IAuthorService
    {
        ...
        public async Task<ICollection<AdminAuthorViewModel>> AllAsync(AdminAllAuthorsViewModel model)
        {
            var authorsQuery = context.Authors.AsNoTracking();
            ...
            if (!string.IsNullOrWhiteSpace(model.SearchString))
            {
                ...
                authorsQuery = authorsQuery.Where(a => EF.Functions.Like(a.Name, wildCard) || 
                                            EF.Functions.Like(a.Id.ToString(), wildCard));
            }

            ICollection<AdminAuthorViewModel> authors = await authorsQuery
                .Where(a => !a.IsDeleted)
                .ProjectTo<AdminAuthorViewModel>(mapper.ConfigurationProvider)
                .ToListAsync();
